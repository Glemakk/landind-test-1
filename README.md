...
1. Перед деплоем `npm i parcel-plugin-nuke-dist`
2. build должен выглядкть так: "build": "parcel build src/\*.html --public-url
   /ИМЯ_РЕПОЗИТОРИЯ/"
3. Редактируем поле homepage https://glemakk.github.io/имя_репозитория
4. npm i gh-pages
5. Добавляем в json "deploy": "gh-pages -d dist", "predeploy": "npm run build"
